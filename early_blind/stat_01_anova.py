...
from v_stat_anova import v_aov
from v_stat_circ_hktest import v_hkt
from joblib import Parallel, delayed

dir_surf = '/gpfs3/well/margulies/users/anw410/data/surfs'
dir_mask = '/gpfs3/well/margulies/users/anw410/data/blindness/serious_2nd/data/mask'

msk_lh = nib.load(dir_mask + '/lh.4grp_mask.func.gii').agg_data()
msk_rh = nib.load(dir_mask + '/rh.4grp_mask.func.gii').agg_data()
...
def v_perm_clus(data, n_perm, angle=False):
    ...
    for i in range(n_perm):
        ...
        #p_perm, _ = circ.watson_williams(data_perm_g1, data_perm_g2, axis=0)
        #p_perm, _ = circ.cmtest(data_perm_g1, data_perm_g2, axis=0)
        #_, p_perm = ss.ttest_ind(data_perm_g1, data_perm_g2, axis=0)
        
        if angle:
            _, p_perm, _ = v_hkt(data_perm, idp, idq, chi_only=True, f_only=False)
            # p_perm[:,msk==0] = 1
            ...
            #_, p_perm = v_anova2(data_perm, idp, idq)
            res = Parallel(n_jobs=-1)(delayed(v_aov)(data_perm, x) for x in range(n_ver))
            res = np.asarray(res)
            p_perm = np.vstack((res[:,1], res[:,3], res[:,5]))
...
idp = np.concatenate((np.zeros(17), np.ones(18), np.zeros(16), np.ones(22))) # factor group: blindness x health
idq = np.concatenate((np.zeros(17+18), np.ones(16+22)))                      # factor site: Montreal x Trento
# data_g1 = np.load(str(sys.argv[1]))
# data_g2 = np.load(str(sys.argv[2]))
# data_g3 = np.load(str(sys.argv[3]))
# data_g4 = np.load(str(sys.argv[4]))
# data_all = np.concatenate((data_g1, data_g2, data_g3, data_g4), axis=0)
...
np.save(dir_save+'/aov_'+pre_fix+'_clus_perm_'+pos_fix+'.npy', clus_max)
