...
for g in range(2):
    ...
    for s in range(len(sbj_lis)):
        ...
        ts_l  = nib.load(dir_out[g]+'/'+sj+'/ses-01/func/'+dir_lab+'/surf/sm4_msk_'+sj+'_hemi-L_surf-fsLR-32k_clean.func.gii').agg_data()
        ts_r  = nib.load(dir_out[g]+'/'+sj+'/ses-01/func/'+dir_lab+'/surf/sm4_msk_'+sj+'_hemi-R_surf-fsLR-32k_clean.func.gii').agg_data()
        ts_lr = np.concatenate((ts_l, ts_r), axis=1)

        ts = ts_lr[:, mask!=0]
        ts_sav = v_ts_regressout(ts, regout)

        np.save(os.path.join(dir_ts, dir_tx0[g], dir_tx1[g], sj+'_clean_reg.npy'), ts_sav)