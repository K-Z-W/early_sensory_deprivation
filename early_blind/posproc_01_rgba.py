...
for g in range(len(dir_tx0)):
    ...
    for s in range(len(sbj_lis)):
        ...
        rgba = v_ts_nnls_vertex(ts_use)
        rgba_all.append(rgba)

    rgba_all = np.asarray(rgba_all)
    print(rgba_all.shape)

    np.save('/gpfs3/well/margulies/users/anw410/data/blindness/serious_3rd/data/rgba/rgba_'+dir_tx0[g]+'_'+dir_tx1[g]+'_235vls_noFIX.npy', rgba_all)