...
def v_save_gifti(data, savepath, savename, half=False):
...
def v_save_cifti_surf(data, savepath, savename, ciftitype='dtseries'):

    n_points, n_brain_locations = np.shape(data)
    if ciftitype == 'dtseries':
        series_axis = SeriesAxis(start=0, step=1, size=n_points)
    elif ciftitype == 'dscalar':
        scalar_axis = ScalarAxis(["Time {}".format(i) for i in range(n_points)])
    
    brain_model_axis_L = BrainModelAxis.from_mask(np.ones(32492, dtype=bool), name="CortexLeft")
    brain_model_axis_R = BrainModelAxis.from_mask(np.ones(32492, dtype=bool), name="CortexRight")
    brain_model_axis = brain_model_axis_L + brain_model_axis_R

    if ciftitype == 'dtseries':
        cifti_img = nib.Cifti2Image(data, (series_axis, brain_model_axis))
        output_filename = os.path.join(savepath, savename+'.dtseries.nii')
    elif ciftitype == 'dscalar':
        cifti_img = nib.Cifti2Image(data, (scalar_axis, brain_model_axis))
        output_filename = os.path.join(savepath, savename+'.dscalar.nii')
    
    cifti_img.to_filename(output_filename)
